...
{
    protected $excludedRequestParams = ['feature_model'];
    ...
    public function store(FeatureRequest $request, Product $product)
    {
        try {
            ...
            if ($request->filled('feature_model')) {
                $feature->models()->create([
                    'model' => $request->get('feature_model'),
                ]);
            }
    ...
    public function update(FeatureRequest $request, Product $product, Feature $feature)
    {
        ...
        try {
            ...
            if ($request->filled('feature_model')) {
                $feature->models()->updateOrCreate(['feature_id' => $feature->id], [
                    'model' => $request->get('feature_model'),
                ]);
            } else {
                $feature->models()->delete();
            }
