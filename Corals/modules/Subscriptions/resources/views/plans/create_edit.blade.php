                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        @forelse($product->features as $feature)
                            @php($plan_feature =  $plan->features()->where('feature_id',$feature->id)->first()?: null)
                            ...
                            @elseif ($feature->type == 'config')
                                {!! CoralsForm::select('features['.$feature->id.'][value][]',$feature->name.' Config',\UsageManager::getSourceValues($feature), true, $plan_feature ? explode(',',$plan_feature->pivot->value):null,['multiple'=>true],'select2') !!}
                            @endif
                            {!! CoralsForm::text('features['.$feature->id.'][plan_caption]','', false, $plan_feature ? $plan_feature->pivot->plan_caption:null,['help_text' => 'Subscriptions::attributes.plan.feature_plan_caption']) !!}
                        @empty
